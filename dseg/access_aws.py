...
if useSandbox:
    print "using sandbox"
    ZProcessLayoutID = "23VZHHRRLFQ0PY1HQ7SKKO9EGTEOFG"
    mturkHost = 'mechanicalturk.sandbox.amazonaws.com'
else:
    print "using real mechanical turk system"
...
def collectDecisions(decisionsToCheck):
    ...
    print "decisionsToCheck", decisionsToCheck


    # initialize decisions
    #decisions = []
    decisionSets = []
    for pair in decisionsToCheck:
        decisionSet = []
    ...
    if stillCollecting:
        print "-------------------------------------------------------"
        print "mechanical turk is still collecting results"
        print "-------------------------------------------------------"

    if not(stillCollecting):
        print "-------------------------------------------------------"
        print "mechanical turk is finished collecting results"
        print "-------------------------------------------------------"
...
class Song(object):
    ...
    def synth(self, filename=None, upload=False, generateMP3=False, tempFilename="temp"):
        ...
        soundFontFile = os.path.join(fileFolder, "WeedsGM3.sf2")
        ...
        try:
            if os.name == 'nt':
                ...
                commandLine = ('fluidsynth/fluidsynth.exe -nli -r 44100 -g 1 -o synth.cpu-cores=2 -F ' + tempFilename + '.wav ' + soundFontFile + ' ' + tempFilename + '.mid')
                print commandLine
